...
<script type="text/javascript">
		function formcheck(){
			...
			var id = regform.userID.value;
			var email = regform.userEmail.value;
			var name = regform.userName.value;
			var gender = $('input[name="userGender"]:checked').val();
			var pw = regform.userPassword.value;
			var pwck = regform.PasswordCheck.value;

			if(id == "" || email =="" || name=="" || pw=="" || pwck==""){
				alert("빈칸을 채워주세요!");
				return false;
			}
			...
			if(id.length<6 || id.length>12){
				alert("id는 6~12자 이내");
				regform.userID.select();
				return false;
			}
			if(pw.length<6 || pw.length>20){
				alert("pw는 6~20자 이내");
				regform.userPassword.select();
				return false;
			}
			if(pwck != pw){
				alert("pw 재확인");
				regform.PasswordCheck.value='';
				regform.userPassword.value='';
				regform.userPassword.select();
				return false;
			}
			return true;
	...
	<div class="container">
		...
		<div class="col-lg-4">
			<div class="jumbotron" style="padding-top: 20px;">
				...
				<form id='frm' method="post" action="joinAction.jsp" onsubmit="return formcheck(); return false;">
					...
					<div class="form-group">
						<input type="password" class="form-control" placeholder="비밀번호확인" name="PasswordCheck"
							maxlength="20">
					</div>
